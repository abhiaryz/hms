...
export default function LoyaltyMemberPage({ params }: { params: { id: string } }) {
  ...
  const fetchMember = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/loyalty?id=${params.id}`);
      if (!response.ok) {
  ...
  const handleAddPoints = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/loyalty?action=points&id=${params.id}`, {
        method: 'POST',
        headers: {
  ...
  const handleRedeemPoints = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/loyalty?action=redeem&id=${params.id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
